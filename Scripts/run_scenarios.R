...
model    <- "century"      # "millennial" / "century" / "MIMICS"
scen     <- read_scenarios("Data/scenarios.csv")
scenario_names = names(scen)

# Run each scenario:
out_century <- lapply(scenario_names, FUN = run_scenario,scen = scen, model = model)

# Get the summary statistics:
lapply(out_century, function(X) X$eq_compare)
...
# Run each scenario:
out_millennial <- lapply(scenario_names, FUN = run_scenario,scen = scen, model = model)

# Get the summary statistics:
lapply(out_millennial, function(X) X$eq_compare)