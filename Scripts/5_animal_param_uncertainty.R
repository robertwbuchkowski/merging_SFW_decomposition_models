...
# ------------------------------------------------------------
unc <- read_param_uncertainty("Data/scenarios.xlsx", category = c("Animal", "Effect"))
...
p_range <- ggplot(plot_df, aes(y = parameter)) +
  ...
  labs(title = "Range of the equilibrium animal effect across animal-parameter uncertainty",
       subtitle = "Bar = effect at parameter lo-hi; dotted line = all-default effect. CV2 = placeholder (no SD/Min-Max).",
